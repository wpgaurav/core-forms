...
class MailChimp extends Action {
    ...
    public function __construct() {
        $this->label = __( 'Mailchimp', 'core-forms' );
    }
    ...
    */
    public function page_settings( $settings, $index ) {
        ...
        <?php if ( ! empty( $selected_list ) ) { ?>
           <span class="cf-action-summary"><?php printf( __( 'Subscribe to %s', 'core-forms' ), $selected_list->name ); ?></span>
    <?php } ?>
       ...
       <table class="form-table">
        <tr valign="top">
            <th scope="row"><?php _e( 'List', 'core-forms' ); ?></th>
            <td>
                <?php if ( ! empty( $lists ) ) { ?>
                    <select name="form[settings][actions][<?php echo $index; ?>][list_id]">
                        <option value="" style="color: #AAA;" readonly><?php _e( 'Select Mailchimp list', 'core-forms' ); ?></option>
                        <?php
                    ...
                    echo '<p><a href="' . admin_url( 'admin.php?page=mailchimp-for-wp' ) . '">' . __( 'Please connect your Mailchimp account first.', 'core-forms' ) . '</a></p>';
                }
                ?>
            </td>
    ...
    public function process( array $settings, Submission $submission, Form $form ) {
        ...
        $merge_fields   = array();
        $merge_fields   = apply_filters( 'cf_mailchimp_action_merge_fields', $merge_fields, $submission, $form );
        $mailchimp_data = array(
            'merge_fields' => $merge_fields,
            'status'       => 'pending',
        );
        $mailchimp_data = apply_filters( 'cf_mailchimp_action_subscriber_data', $mailchimp_data, $submission, $form );
        ...
        $name = sprintf( 'Core Forms: %s', $form->title );
        if ( ! $result ) {
